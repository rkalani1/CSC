...
This chapter is the scorecard manual. It lists the GWTG achievement measures and Target: Stroke thresholds as published award criteria, the STK inpatient set, and CSTK-01 through CSTK-12 with CSTK-07 omitted because it is not in the current set. It then describes how to run a monthly scorecard, how to set `internal` targets above those criteria, and how to operate 90-day mRS capture as a production process.

Do not manage to last year's award. Manage to this month's denominators, exclusions, and missing data.

Use Chapter 22 to keep the data true, Chapter 24 to improve defects, Chapter 25 to show surveyors the same numbers, and Chapter 26 to stratify every row.

## Why This Matters

Joint Commission DSC certification requires performance measurement. For a CSC, that means CSTK and the longstanding STK set. The operational CSTK window is v2026B (3Q–4Q 2026); v2027A is posted for 1Q–2Q 2027; IDs 01–06 and 08–12 are unchanged; CSTK-07 is still absent. GWTG-Stroke is the national registry and the public recognition program most academic CSCs use to benchmark and to structure hospital-facing awards. The 2026 AHA/ASA AIS Guideline is the clinical source that makes several of these measures clinically coherent: treat eligible disabling deficits rapidly within 4.5 hours regardless of NIHSS; give IVT and EVT together when both are indicated, without delaying thrombectomy; use tenecteplase 0.25 mg/kg IV push (max 25 mg) or alteplase 0.9 mg/kg (max 90 mg) in the 4.5-hour window.

Published award criteria are not CSC certification floors and not elite performance. GWTG achievement awards require 85% on each of seven achievement measures. Target: Stroke Honor Roll, Elite, Elite Plus, and Advanced Therapy are **published award criteria**, not CSC certification floors. Label any local DTN or door-to-device aim `internal`. An academic CSC that treats 85% and Honor Roll 75% as aspirations will be average on a good month and invisible on a bad one. Never let monthly quality confuse award criteria with internal aims.
...
Run one monthly book with four pages: GWTG achievement and Target: Stroke; STK; CSTK; internal aims and equity. Every row has a definition owner, a numerator, a denominator, a published award criterion if one exists, an `internal` target, and a color that means something.
...
flowchart TB
    subgraph Pages["Monthly scorecard book"]
      ...
      P4[Page 4 Internal aims and equity]
    end
...
| # | Achievement measure | Published award criterion | What the Medical Director actually manages |
| --- | --- | --- | --- |
...
| Recognition | Published award criterion | Internal posture |
| --- | --- | --- |
| Honor Roll | **75% DTN ≤60 min** | Published award criterion; not a certification floor |
| Elite | **85% DTN ≤60 min** | Published award criterion; a typical `internal` DTN aim if so labeled |
| Elite Plus | **75% DTN ≤45 min AND 50% DTN ≤30 min** | Published award criterion; label any local tighter aim `internal` |
| Advanced Therapy | **50% door-to-device ≤90 min direct / ≤60 min transfer** | Related to, not identical with, CSTK-09 (ER-TS-AT-DEF) |

Report DTN and door-to-device with numerator, denominator, median, and the percent at each published cut. A program can meet Elite on percent ≤60 minutes and still have a long tail that CSTK-11 will punish. Always show the tail. Advanced Therapy door-to-device is arrival to first pass with the device; CSTK-09 is arrival to skin puncture.
...
### CMS Care Compare (confirm live)

CMS public reporting is a third book, not a fourth GWTG metal. **MORT-30-STK** (30-day risk-standardized ischemic-stroke mortality) appears on Care Compare — confirm live whether the hospital is in the current display set. STK eCQMs used in IQR, if any, are confirm-live against the current CMS eCQM list; do not assume the Joint Commission STK chart-abstracted set is what CMS displays. OP-23 historically tracked head CT/MRI results for stroke; do not assert it as mandatory. GWTG awards ≠ CMS stars ≠ the CSC certificate. A Gold Plus month does not move a star rating, and a CSC plaque does not correct a MORT-30-STK outlier.
...
| --- | --- | --- | --- |
| CSTK-01 | NIHSS performed for ischemic stroke | NIHSS documented before recanalization, or within 12 hours of arrival if no recanalization. Sampling: Yes | Ischemic |
| CSTK-02 | mRS at 90 days | A 90-day mRS is obtained on eligible patients | Ischemic follow-up operation |
| CSTK-03 | Severity measurement for SAH and ICH (overall) | Hunt and Hess for SAH (03a); ICH Score for ICH (03b); prior to surgery or ≤6 h of arrival if no surgery (ER-CSTK-03T). WFNS and modified Fisher are clinical adjuncts, not the measure. Sampling: Yes | Hemorrhagic |
| CSTK-04 | Procoagulant reversal agent initiation for ICH | ICH with INR ≥1.4 at arrival; numerator agents PCC, FFP, or rFVIIa (ER-CSTK-04D). Class-specific DOAC reversal is a clinical SOP, not this numerator | ICH |
| CSTK-05 | Hemorrhagic Transformation (Overall Rate) | sICH: NIHSS ↑ ≥4 and PH, SAH, or IVH ≤36 h after treatment (v2026B extraction; confirm live). 05a/05b are therapy strata — confirm live. v2026B MIF still names alteplase; confirm live whether TNK is in the denominator | Ischemic with reperfusion safety |
| CSTK-06 | Nimodipine treatment administered | First dose within 24 h of arrival (ER-CSTK-06T) | aSAH |
| CSTK-08 | TICI post-treatment reperfusion grade | A post-treatment TICI grade is documented | MER |
| CSTK-09 | Arrival time to skin puncture (overall) | Arrival to skin puncture. 09a = transfers; 09b = direct arrivals or undocumented mode (confirm live MIF0343). Sampling: Yes. Pair with Advanced Therapy 90/60; related, not identical (ER-TS-AT-DEF) | EVT |
...
CSTK-01, CSTK-03, and CSTK-09 have Sampling: Yes. Confirm the current sample-size tables before each submission cycle. Concurrent review of NIHSS, severity scores, and puncture times remains a population activity even when a sample is submitted.
...
**How to read the reperfusion cluster.** CSTK-08 asks whether a grade exists. CSTK-09 asks how long to puncture (09a transfers; 09b direct arrivals or undocumented mode — confirm live MIF0343). CSTK-11 asks whether effective reperfusion (TICI ≥2B) happened within 120 minutes of arrival. CSTK-12 asks whether it happened within 60 minutes of puncture. A center can pass CSTK-08 and fail CSTK-11 (grade documented, too late from the door). A center can pass CSTK-11 and fail CSTK-12 (fast from the door, slow in the lab). A center can pass CSTK-12 and fail CSTK-11 (efficient lab, late door-to-puncture). Manage all four. Pair them with Target: Stroke Advanced Therapy (50% door-to-device ≤90 min direct / ≤60 min transfer). The definitions are related, not identical: Advanced Therapy clocks first device pass; CSTK-09 clocks skin puncture (ER-TS-AT-DEF).

**How to read the safety and hemorrhagic cluster.** CSTK-05 is a safety outcome (sICH as defined in the table; confirm live). Do not "improve" it by under-ascertaining or by reclassifying clinical decline. Pair every CSTK-05 case with peer review and with the defect taxonomy in Chapter 24. CSTK-03 uses Hunt and Hess (SAH) and ICH Score (ICH), not WFNS. CSTK-04 clocks INR ≥1.4 ICH reversal with PCC, FFP, or rFVIIa; class-specific DOAC reversal is the clinical SOP, not the numerator. CSTK-06 clocks first nimodipine within 24 h of arrival. The April 2, 2025 Joint Commission announcement reduced the annual aSAH volume criterion to 10; volume is not a substitute for CSTK-06 reliability.
...
### Internal targets versus published award criteria

Write the distinction into the scorecard header so a new monthly-quality member cannot miss it.

| Domain | Published award criterion / specification | Internal target (example posture; label `internal`) |
| --- | --- | --- |
| GWTG achievement (each of 7) | 85% | 95%, with case-level review of every miss |
| DTN ≤60 min | Honor Roll 75%; Elite 85% | ≥90% ≤60 min |
| DTN ≤45 / ≤30 min | Elite Plus 75% / 50% | Meet the Elite Plus cuts every rolling 6 months, not as a one-month spike |
| Door-to-device | Advanced Therapy 50% at 90/60 | A locally set percent above 50%, plus CSTK-11/12 reliability |
| CSTK-01 | As specified; Sampling: Yes | Unsampled `internal` compliance near 100% before recanalization |
...
3. Time: DTN (Honor Roll / Elite / Elite Plus published cuts), door-to-device (Advanced Therapy published cuts), CSTK-09/11/12.
...
**After the meeting.** Publish a one-page decision log to the monthly quality list. Update the operational dashboard owners. Do not wait for the next monthly to start a time-critical PDSA.
...
- Add the patient to the 90-day list at discharge, not at day 80. Execute day-60 first attempt / day-75 second attempt / spec-window close (confirm v2026B).
...
!!! tip "Key Actions"
    Publish a one-page scorecard dictionary that maps each GWTG, STK, and CSTK row to a source report and a human owner. Remove CSTK-07 from every local artifact. Pair CSTK-02 with CSTK-10 on every slide. Pair CSTK-08, 09, 11, and 12 on every reperfusion discussion. Set `internal` targets above 85% achievement and above the published Honor Roll and Advanced Therapy cuts. Stand up the 90-day worklist as a daily queue with an aging report.

!!! abstract "Metrics Targets"
    GWTG achievement: 85% on each of the seven measures; Bronze 90 days, Silver 12 months, Gold 24 months. Target: Stroke (min 6 patients) Honor Roll / Elite / Elite Plus / Advanced Therapy cuts as tabulated above — **published award criteria**, not CSC certification floors. Target: Type 2 Diabetes composite ≥80% for 12 months. CSTK-11: TICI ≥2B within 120 min of arrival. CSTK-12: TICI ≥2B within 60 min of puncture. STK-1 through 6, 8, and 10 as specified. `Internal` targets sit above these criteria and are local policy. CSTK-01, CSTK-03, and CSTK-09: Sampling: Yes; confirm current sample sizes.

!!! warning "Common Pitfalls"
    Managing to Gold while ignoring CSTK-11/12. Reporting CSTK-10 without CSTK-02. Sampling only CSTK-01 when CSTK-03 and CSTK-09 also have Sampling: Yes. Using Honor Roll 75% as the unlabeled `internal` DTN goal. Letting a vendor tile still display CSTK-07. Treating STK-4 as agent-agnostic folklore after the 2026 AIS Guideline. Celebrating a short median DTN that hides a long tail. Interpreting a month with n=4 as Elite Plus. Allowing "doing well" to pass as a 90-day mRS. Setting an sICH "improvement target" that punishes reporting. Collapsing CSTK-09 puncture with Advanced Therapy first-pass. Assuming TNK is in the CSTK-05 denominator because the house agent changed.

!!! success "Implementation Tips"
    Read exceptions, not green ink. Put data quality as item 1 so monthly quality cannot skip it. Attach huddle notes to in-month outliers so the monthly meeting is adjudication, not discovery. Teach fellows the difference between CSTK-11 and CSTK-12 with two real cases. When a measure moves after a specification update, annotate the chart rather than launching a PDSA on a definition change. Keep a rolling 12-month view next to the single month so small denominators do not drive theater.

## How to Do the Work

### Daily / weekly

- Scan the operational list for DTN >60 minutes, DTN misses against the 45- and 30-minute cuts, Advanced Therapy door-to-**device** misses against 90/60, CSTK-09 puncture misses, missing pre-recanalization NIHSS, missing TICI, new CSTK-05 flags, and ICH reversal or nimodipine delays.
...
Weekly ops uses the concurrent list. Monthly quality uses the locked book. Do not confuse the two meetings.

### Monthly / quarterly

- Lock and run the four-page scorecard as specified above.
- Recalculate Target: Stroke status on a rolling basis against the published Honor Roll / Elite / Elite Plus / Advanced Therapy criteria, with explicit denominators.
...
- Confirm CSTK-01, CSTK-03, and CSTK-09 sampling (if used) still matches the current manual.
- Quarterly, present rolling 12-month performance to quarterly stroke executive, with published award criteria and `internal` targets labeled.
...
| Row | n | Num | Den | % | Published award criterion | Internal target | Status | Owner | Action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
3. DTN and door-to-device versus published Honor Roll / Elite / Elite Plus / Advanced Therapy criteria (10).
...
4. Execute day-60 first attempt / day-75 second attempt / spec-window close (confirm v2026B).
5. Execute the method hierarchy.
6. Enter mRS, date, method, and examiner as discrete fields.
7. Record unsuccessful attempts.
8. Close inside the allowable window or document why not.
9. Monthly audit of a locally defined sample.
...
"Published award criteria are GWTG 85% achievement and the Target: Stroke Honor Roll / Elite / Elite Plus / Advanced Therapy cuts. CSTK/STK specifications are certification measures, not awards. Internal targets are set by monthly quality (operational aims) and quarterly stroke executive (charter), labeled `internal`, and may be stricter. No staff member shall present an `internal` target as a certification requirement. No sICH rate target shall be written in a way that discourages complete CSTK-05 ascertainment."
...
- Specifications Manual for Joint Commission National Quality Measures, CSTK v2026B (posted 02/06/2026; 3Q–4Q 2026 discharges): CSTK-01 through 06 and 08 through 12; CSTK-07 not in the current set. v2027A posted for 1Q–2Q 2027; IDs unchanged. CSTK-01, CSTK-03, and CSTK-09 Sampling: Yes. CSTK-03 Hunt and Hess (03a) / ICH Score (03b), pre-surgical or ≤6 h (ER-CSTK-03T). CSTK-04 INR ≥1.4; PCC/FFP/rFVIIa (ER-CSTK-04D). CSTK-05 Hemorrhagic Transformation (Overall Rate); sICH definition as v2026B extraction, confirm live; TNK-in-denominator confirm live. CSTK-06 first dose within 24 h of arrival. CSTK-09 09a transfers / 09b direct or undocumented mode (confirm live MIF0343).
- STK inpatient measure set (STK-1, 2, 3, 4, 5, 6, 8, 10); STK-OP v2026B. CMS Care Compare: MORT-30-STK confirm live; STK eCQMs confirm live; OP-23 not asserted as mandatory. GWTG awards ≠ CMS stars ≠ CSC certificate.
...
- The Joint Commission. 2026 Stroke Certification Standards (SCS26). The April 2, 2025 Joint Commission announcement reduced the annual aSAH volume criterion to 10.
